...
class EnglishController {
    async addEnData(ctx, next) {
        const { english, englishCn, enClass, university, CTE4, CTE6 } = ctx.request.body
        try {
            const res = await addEn(english, englishCn, enClass, university, CTE4, CTE6);
            ctx.body = {
                code: 0,
                message: '单词添加成功',
                result: {
    ...
    async addEnDataS(ctx, next) {
        ...
        for (const EnData in EnList) {
            const { english, englishCn, enClass, university, CTE4, CTE6 } = EnList[EnData]
            try {
                await addEn(english, englishCn, enClass, university, CTE4, CTE6);
                s++;
            } catch (error) {
                e++;
                console.log(error)
